...
namespace renderer {
	...
	Screen::Screen()
	{
		this->width = 800;
		this->height = 600;
		this->center = math::Point2d(0, 0); // when not specified the center of the canvas matches the origin in global 2D/UI world space coordinates
	...
	Screen::Screen(int width, int height, math::Point2d center) : width(width), height(height), center(center)
	{
		initTransforms();
	}
	...
	math::Point2d Screen::toCanvasWorldCoords(math::Point2d p, Canvas canvas)
	{
		return getScreenToCanvasWorldTransform(canvas) * p;
	}

	math::Point2d Screen::toCanvasCoords(math::Point2d p, Canvas canvas)
	{
		// TODO: take into account the eventual difference in scale between canvas and screen
	...
	math::Point2d* Screen::getPoint(int x, int y)
	{
